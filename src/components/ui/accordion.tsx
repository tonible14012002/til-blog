...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Root
    className={clsx('p-2 space-y-1 bg-white rounded-lg border', className)}
    ref={ref}
    {...props}
  />
))
...
>((props, ref) => {
  ...
  const renderRightIcon = (
    <span className={iconStyle} id={!rightIcon ? 'chevron' : undefined}>
      {!rightIcon ? <ChevronDown size={16} /> : rightIcon}
    </span>
  )
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        className={clsx(
          'flex-1 flex font-medium text-sm items-center justify-between [&[data-state=open]>#chevron]:rotate-180 ease-in-out gap-3 p-2',
          wrapperClassName,
        )}
        ref={ref}
        {...restProps}
...
>((props, ref) => {
  ...
  return (
    <AccordionPrimitive.Content
      className={clsx('text-sm overflow-hidden data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down', wrapperClassName)}
      ref={ref}
      {...restProps}
    >
      <div
